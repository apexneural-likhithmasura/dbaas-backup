...
try:
    # Add local fastapi2-rahul-reddit folder to path
    current_dir = os.path.dirname(os.path.abspath(__file__))
    reddit_path = os.path.join(current_dir, "fastapi2-rahul-reddit")
    ...
    if os.path.exists(reddit_path):
        ...
        print("✅ Reddit components loaded successfully from fastapi2-rahul-reddit/")
    else:
...
async def _run_complete_pipeline(
    ...
    try:
        if not REDDIT_AVAILABLE:
            raise RuntimeError("Reddit components not available. Please ensure fastapi2-rahul-reddit folder exists with all required files.")
...
async def root():
    ...
    return {
        ...
        "endpoints": [
            ...
            "/generate-prompt",
            "/analyze-reddit-post",
            "/reddit/search-and-rank",
            "/reddit/scrape-posts"
        ],
        "features": {
            "ai_powered_expansion": "Claude 3.5 Sonnet",
            "reddit_scraping": "Automated search and analysis",
            "reddit_search_and_rank": "Search and AI-rank Reddit discussions",
            "reddit_scrape_posts": "Scrape full data from Reddit posts",
...
async def start_complete_pipeline(request: CompletePipelineRequest):
    ...
    if not REDDIT_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="Reddit components not available. Please ensure fastapi2-rahul-reddit folder exists with google_search.py, reddit_scraper.py, and openai_ranker.py"
        )
...
@app.post("/reddit/search-and-rank")
async def reddit_search_and_rank(request: RedditSearchRequest):
    """
    Search Reddit for a market topic and return AI-ranked posts.
    
    This endpoint:
    1. Searches for Reddit discussions related to the market topic
    2. Scrapes metadata (title, upvotes, comments) from found posts
    3. Uses AI to rank posts by product development potential
    
    Args:
        request: RedditSearchRequest with market query and parameters
        
    Returns:
        JSON response with ranked Reddit posts
    """
    if not REDDIT_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="Reddit components not available. Please ensure fastapi2-rahul-reddit folder exists with all required files."
        )
    
    try:
        logger.info(f"Starting Reddit search and rank for market: {request.market}")
        ...
        logger.info(f"Searching Reddit for: {request.market}")
        urls = await asyncio.to_thread(
            searcher.search_market, 
            request.market, 
            request.num_results
        )
        ...
        if not urls:
            raise HTTPException(
                ...
                detail={
                    "status": "error",
                    "message": f"No Reddit posts found for market: {request.market}"
                }
            )
        
        logger.info(f"Found {len(urls)} Reddit URLs")
        
        # Step 2: Scrape metadata from posts
        logger.info("Scraping post metadata...")
        posts_data = await asyncio.to_thread(
            scraper.scrape_multiple_posts, 
            urls
        )
        ...
        if request.use_ai_ranking:
            api_key = os.getenv("OPENROUTER_API_KEY")
            if not api_key:
                logger.warning("OPENROUTER_API_KEY not found, using fallback ranking")
                ranker = PostRanker(api_key="dummy")
                ranked_posts = await asyncio.to_thread(
                    ranker._fallback_ranking,
                    posts_data,
                    request.top_n
                )
            else:
                ranker = PostRanker(api_key)
                logger.info("Using AI to rank posts...")
                ranked_posts = await asyncio.to_thread(
                    ranker.rank_posts,
                    posts_data,
                    request.market,
        ...
        return {
            "status": "success",
            "message": f"Found and ranked {len(ranked_posts)} Reddit posts",
            "data": {
                "market": request.market,
                "total_posts_found": len(urls),
                "ranked_posts": ranked_posts,
                "ranking_method": "ai" if request.use_ai_ranking and os.getenv("OPENROUTER_API_KEY") else "engagement_metrics",
                "timestamp": datetime.now().isoformat()
            }
        }
        ...
        logger.error(f"Error in reddit_search_and_rank: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail={
                "status": "error",
                "message": "Failed to search and rank Reddit posts",
                "error": str(e)
            }
        )


@app.post("/reddit/scrape-posts")
async def reddit_scrape_posts(request: RedditPostUrlsRequest):
    """
    Scrape full data from specific Reddit post URLs.
    
    This endpoint:
    1. Takes a list of Reddit post URLs
    2. Scrapes metadata (title, upvotes, comments)
    3. Optionally extracts full JSON data with post content and all comments
    
    Args:
        request: RedditPostUrlsRequest with URLs and extraction options
        
    Returns:
        JSON response with scraped Reddit post data
    """
    if not REDDIT_AVAILABLE:
        raise HTTPException(
            status_code=503,
    ...
    for url in request.urls:
        if 'reddit.com' not in url or '/comments/' not in url:
            raise HTTPException(
                ...
                detail={
                    "status": "error",
                    "message": f"Invalid Reddit post URL: {url}"
                }
            )
    
    try:
        logger.info(f"Scraping {len(request.urls)} Reddit posts")
        ...
        for url in request.urls:
            ...
            post_data = {
                ...
                "metadata_only": not request.extract_full_json
            }
            
            # Get full JSON if requested
            if request.extract_full_json:
                logger.info(f"Extracting full JSON for: {url}")
                json_data = await asyncio.to_thread(
                    scraper.get_post_json,
                    url
                )
                ...
                if json_data:
                    ...
                    structured_data = await asyncio.to_thread(
                        scraper.extract_post_and_comments,
                        json_data
                    )
                    
                    if structured_data:
                        post_data["full_data"] = structured_data
                        post_data["extraction_success"] = True
                    else:
                        post_data["extraction_success"] = False
                        post_data["error"] = "Failed to parse JSON data"
                else:
                    post_data["extraction_success"] = False
                    post_data["error"] = "Failed to fetch JSON data"
            
            scraped_posts.append(post_data)
        
        return {
            "status": "success",
            "message": f"Successfully scraped {len(scraped_posts)} Reddit posts",
            "data": {
                "posts": scraped_posts,
                "total_posts": len(scraped_posts),
                "full_extraction": request.extract_full_json,
                "timestamp": datetime.now().isoformat()
            }
        }
        ...
        logger.error(f"Error in reddit_scrape_posts: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail={
                "status": "error",
